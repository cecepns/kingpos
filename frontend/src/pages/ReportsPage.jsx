    ...
    <PageStackLoose>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        <div className="w-full sm:w-44">
          ...
          <div className="mt-1">
            <AppDatePicker value={from} onChange={(val) => setFrom(val)} />
          </div>
        </div>
        <div className="w-full sm:w-44">
          <label className="text-xs font-medium text-slate-500">Sampai</label>
          <div className="mt-1">
            <AppDatePicker value={to} onChange={(val) => setTo(val)} />
          </div>
        </div>
        <button
          type="button"
          onClick={() => exportPdf()}
          className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-soft transition hover:bg-red-700"
        >
