...
import { Pencil, Plus, Trash2, UserX, Landmark } from "lucide-react";
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl bg-slate-800 px-5 py-2.5 font-semibold text-white shadow-soft transition hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600"
          >
            <Landmark className="h-4 w-4" /> Kelola rekening kas
          </button>
          <button
            type="button"
            onClick={() => {
            ...
            className="inline-flex items-center gap-2 rounded-2xl bg-brand-600 px-5 py-2.5 font-semibold text-white shadow-soft transition hover:bg-brand-700"
          >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div className="w-full sm:w-44">
            ...
            <AppDatePicker value={tanggalDari} onChange={(val) => setTanggalDari(val)} />
          </div>
          <div className="w-full sm:w-44">
            <label className="mb-1 block text-xs text-slate-500">Sampai tanggal</label>
            <AppDatePicker value={tanggalSampai} onChange={(val) => setTanggalSampai(val)} />
          </div>
          ...
          <div>
            ...
            <div className="mt-1">
              <Controller
                ...
                name="flow_date"
                render={({ field }) => (
                  <AppDatePicker value={field.value} onChange={(val) => field.onChange(val)} />
                )}
              />
            </div>
          </div>
            ...
            <div>
              ...
              <div className="mt-1">
                <AppDatePicker
                  ...
                  onChange={(val) => setEditFlow((s) => ({ ...s, flow_date: val }))}
                />
              </div>
            </div>
            <div className="md:col-span-2">
