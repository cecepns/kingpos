...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!product) {
      // Try searching in variants
      const [vRows] = await pool.query(
        `SELECT pv.*, p.name AS product_name, p.image_path FROM product_variants pv JOIN products p ON p.id = pv.product_id WHERE (pv.barcode = ? OR pv.sku = ?) AND p.is_active = 1 LIMIT 1`,
        [code, code]
      );
      if (vRows.length) {
        matchedVariant = vRows[0];
        const [pRows] = await pool.query(`SELECT id, name, sku, barcode, sell_price, wholesale_price, wholesale_min_qty, stock, image_path, is_active FROM products WHERE id=?`, [matchedVariant.product_id]);
        product = pRows[0];
      }
    }
    ...
    const [variants] = await pool.query(`SELECT * FROM product_variants WHERE product_id=? ORDER BY id ASC`, [product.id]);
    const [tiers] = await pool.query(`SELECT * FROM product_tiers WHERE product_id=? ORDER BY min_qty ASC`, [product.id]);
    res.json({ product, variants, matchedVariant, tiers: tiers || [] });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [r] = await pool.query(
      `INSERT INTO products (sku, barcode, name, description, supplier_id, purchase_price, sell_price, wholesale_price, wholesale_min_qty, stock, min_stock, unit, location, brand, is_active)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        sku,
        barcode,
    ...
    if (Array.isArray(b.tiers)) {
      for (const t of b.tiers) {
        ...
        await pool.query(`INSERT INTO product_tiers (product_id, min_qty, price) VALUES (?,?,?)`, [
          pid,
          Number(t.min_qty),
          Number(t.price),
        ]);
    ...
    if (Array.isArray(b.variants)) {
      for (const v of b.variants) {
        ...
        await pool.query(
          `INSERT INTO product_variants (product_id, name, sku, barcode, sell_price, wholesale_price, wholesale_min_qty, stock) VALUES (?,?,?,?,?,?,?,?)`,
          [
            pid,
            String(v.name).trim(),
            v.sku ? String(v.sku).trim() : null,
            v.barcode ? String(v.barcode).trim() : null,
            Number(v.sell_price || b.sell_price || 0),
            Number(v.wholesale_price || 0),
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    params.push(pid);
    await pool.query(
      `UPDATE products SET sku=?, barcode=?, name=?, description=?, supplier_id=?, purchase_price=?, sell_price=?, wholesale_price=?, wholesale_min_qty=?,
       min_stock=?, unit=?, location=?, brand=?, is_active=?${stockPart} WHERE id=?`,
      params
    );
    await pool.query(`DELETE FROM product_categories WHERE product_id=?`, [pid]);
    if (Array.isArray(b.category_ids)) {
      for (const cid of b.category_ids) {
        await pool.query(`INSERT INTO product_categories (product_id, category_id) VALUES (?,?)`, [pid, cid]);
      }
    }
    await pool.query(`DELETE FROM product_tiers WHERE product_id=?`, [pid]);
    if (Array.isArray(b.tiers)) {
      for (const t of b.tiers) {
        if (!t.min_qty || !t.price) continue;
        await pool.query(`INSERT INTO product_tiers (product_id, min_qty, price) VALUES (?,?,?)`, [
    ...
    if (Array.isArray(b.variants)) {
      await pool.query(`DELETE FROM product_variants WHERE product_id=?`, [pid]);
      for (const v of b.variants) {
        if (!v.name || !String(v.name).trim()) continue;
        await pool.query(
          `INSERT INTO product_variants (product_id, name, sku, barcode, sell_price, wholesale_price, wholesale_min_qty, stock) VALUES (?,?,?,?,?,?,?,?)`,
...
async function createPosTransaction(body, userId, conn) {
  ...
  for (const lr of lineRows) {
    ...
    if (status === "completed") {
      if (lr.variant_id) {
        await conn.query(`UPDATE product_variants SET stock = GREATEST(0, stock - ?) WHERE id=?`, [lr.qty, lr.variant_id]);
      }
    }
  }
